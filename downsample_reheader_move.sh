...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 [Desired Final Coverage] [Contaminant Proportion] [BAM File 1] [BAM File 2] [Output Directory]"
    exit 1
fi
...
calculate_coverage() {
    ...
    ./mosdepth -t 2 -n -x -Q 1 "${output_dir}/${prefix}" "$bam_file"
    local mean_coverage=$(awk '{print $4}' "${output_dir}/${prefix}.mosdepth.summary.txt")
...
samtools view -b -s $downsample_percentage_1 $bam_file_1 > "$output_dir/$output_bam_1"
echo "$bam_file_1 downsampled to $downsample_percentage_1%."
samtools index "$output_dir/$output_bam_1"

echo "Downsampling $bam_file_2 to $downsample_percentage_2..."
samtools view -b -s $downsample_percentage_2 $bam_file_2 > "$output_dir/$output_bam_2"
echo "$bam_file_2 downsampled to $downsample_percentage_2%."
samtools index "$output_dir/$output_bam_2"

# Additional checks for final downsampled BAM files
echo "Calculating final coverage for downsampled BAM files..."
final_coverage_1=$(calculate_coverage "$output_dir/$output_bam_1")
final_coverage_2=$(calculate_coverage "$output_dir/$output_bam_2")
echo "Final coverage for $output_bam_1: $final_coverage_1"
echo "Final coverage for $output_bam_2: $final_coverage_2"

echo "Downsampling and coverage calculation complete. Files have been moved and indexed in $output_dir."
echo "Final coverages after downsampling: $final_coverage_1 (for $output_bam_1), $final_coverage_2 (for $output_bam_2)."
