...
>>>
  ...
  output {
      ...
      File output_main_bam_index = sub(main_bam_file, "\\.bam$", "") + "_~{desired_final_coverage}_downsampled.bai"
      File output_contaminant_bam = sub(contaminant_bam_file, "\\.bam$", "") + "_~{contaminant_proportion}_downsampled.bam"
      File output_contaminant_bam_index = sub(contaminant_bam_file, "\\.bam$", "") + "_~{contaminant_proportion}_downsampled.bai"
      File reheadered_contaminant_bam = "~{main_sample_name}_~{contaminant_proportion}_reheadered.bam"
      File reheadered_contaminant_bam_index = "~{main_sample_name}_~{contaminant_proportion}_reheadered.bai"
      File merged_bam = "~{merged_output_filename}"
      File merged_bam_index = "~{merged_output_filename}.bai"
  }
}
